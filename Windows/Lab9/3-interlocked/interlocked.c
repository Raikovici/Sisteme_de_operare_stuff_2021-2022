...
DWORD WINAPI thread_function(LPVOID arg)
{
	...
	for (i = 0; i < NO_ROUNDS; i++) {
		 ...
		 */
		InterlockedIncrement(&count); //incrementez in mod atomic variabila
		//printf("\tIncrement count at round %d\n", i);
		SwitchToThread();
		InterlockedCompareExchange(&count, 0, LIMIT); //compar cu LIMIT si ii schimb valoarea la 0 daca e adevarat
		//printf("\tCoompare at round %d\n", i);
		/* TODO 3:
...
DWORD WINAPI thread_function_mutex(LPVOID arg)
{
	...
	for (i = 0; i < NO_ROUNDS; i++) {
		 ...
		 */
		WaitForSingleObject(hMutex, INFINITE); //ocup mutexul
		count++;
		if (count == LIMIT)
			count = 0;
		ReleaseMutex(hMutex); //eliberez mutexul
	}
