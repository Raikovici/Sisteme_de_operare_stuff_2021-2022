...
DWORD CreateThresholdBarrier(THB_OBJECT* pthb, DWORD b_value)
{
	...
	*pthb = hthb;
	...
	DIE(hthb->hGuard == INVALID_HANDLE_VALUE, "cannot create mutex");
	/* TODO - Create Event */
...
DWORD WaitThresholdBarrier(THB_OBJECT thb)
{
	...
	WaitForSingleObject(thb->hGuard, INFINITE); //wait un thread

	thb->dwCount++; //a sosit un thread
	printf("Thread %d is in barrier\n", GetCurrentThreadId());
	if (thb->dwCount < thb->dwThreshold) //daca numaru de threaduri e mai mic decat nr maxim de threaduri ce pot fi in bariera
	{
		SignalObjectAndWait(thb->hGuard, thb->hEvent, INFINITE, FALSE); //astept mai multe threaduri
		//semnalez mutexu si astept pe event sa se intample ceva
	}
	else //altfel, daca am atins maximum
	{
		thb->dwCount = 0; //reinitializez countul
		PulseEvent(thb->hEvent); //deblochez toate threadurile care asteapta la eveniment
		ReleaseMutex(thb->hGuard); //deblochez si mutexu, n are sens sa l mai tin ocupat
	}
	return 0;
}
