...
static void init_io_async(void)
{
	...
	iocp = iocp_init(); //initializez portul
		
	/* TODO - add handles to completion port */
	for (i = 0; i < n_files; i++)
	{
		temp = iocp_add(iocp, fds[i]); //adaug fiecare handle la completion port
		DIE(temp == NULL, "iocp add");
	}
}
...
static void wait_io_async(void)
{
	...
	for (i = 0; i < n_files; i++) {
		...
		bRet = iocp_wait(iocp, &bytes, &key, &op); //astept dupa fiecare operatie asincrona
