...
static int server(void)
{
	...
	efd = epoll_create(CLIENT_COUNT); //creez epoll care sa cuprinda toti clientii
	DIE(efd < 0, "server: epoll create failed");

	ev.events = EPOLLIN;
	ev.data.fd = event_fd;
	rc = epoll_ctl(efd,EPOLL_CTL_ADD, event_fd,&ev);
	DIE(rc < 0, "server: epoll adding eventfd failed");
	

	for (i = 0; i < CLIENT_COUNT; i++) {
		ev.events = EPOLLIN; //mentionez ca e vorba de date la citire
		ev.data.fd = pipes[i][PIPE_READ]; //folosesc capatu de read
		
		rc = epoll_ctl(efd,EPOLL_CTL_ADD, pipes[i][PIPE_READ],&ev); //adaug capatu de citire la epoll
		DIE(rc < 0, "server: epoll adding pipe failed");
		 
		rc = close(pipes[i][PIPE_WRITE]); //inchid capatul de scriere
	...
	while (recv_msgs < CLIENT_COUNT) {
		...
		if((ev.events & EPOLLIN) != 0){
			if(ev.data.fd == event_fd){
				rc = read(ev.data.fd, &event, sizeof(uint64_t));
				DIE(rc < 0, "server: read magic exit failed");
					
				if(((event & MAGIC_EXIT)>>32) == (MAGIC_EXIT >> 32)){
					index = get_index(event);

					rc = epoll_ctl(efd,EPOLL_CTL_DEL, pipes[index][PIPE_READ], NULL);
					DIE(rc < 0, "server: epoll delete failed");
				...
				DIE(rc < 0, "server: epoll add failed");
	...
	rc = epoll_ctl(efd,EPOLL_CTL_DEL, event_fd, NULL);
	DIE(rc < 0, "server: epoll delete eventfd failed");
...
static int client(unsigned int index)
{
	...
	DIE(rc < 0, "write magic exit failed");
...
int main(void)
{
	...
	DIE(event_fd < 0, "eventfd create failed");
#endif
