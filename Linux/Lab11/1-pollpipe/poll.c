...
static int server(void)
{
	...
	for (i = 0; i < CLIENT_COUNT; i++) {
		rc = close(pipes[i][PIPE_WRITE]); //inchid capetele de scriere, n am nevoie de ele
		DIE(rc < 0, "server: close pipe failed");
		pdf[i].fd = pipes[i][PIPE_READ]; //capat de citire
		pdf[i].events = POLLIN;  //exista date ce tre sa fie citite
	}
	...
	while (recv_msgs < CLIENT_COUNT) {
		...
		for (i = 0; i < CLIENT_COUNT; i++) {
			if ((pdf[i].revents & POLLIN) != 0) { //daca mai sunt evenimente 
				recv_msgs++; //incrementez nr de mesaje citite
				recv_count = read(pipes[i][PIPE_READ], msg, //citesc din fiecare capat de read al pipe ului
						MSG_SIZE);
