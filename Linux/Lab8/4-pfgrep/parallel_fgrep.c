...
int main(int argc, char **argv)
{
	...
	for( i = 0; i < NUM_THREADS; i++) {
		rc = pthread_create(&th_id[i], NULL, thread_code, &th_params[i]); //creez 4 threaduri care vor executa in paralel functia thread_code
		DIE(rc == -1, "cannot create thread");
	}

	/* TODO - wait for threads to finish and collect results */
	for (i = 0; i < NUM_THREADS; i++) {
		rc = pthread_join(th_id[i], &results[i]); //astept ca fiecare thread sa se termine si tin minte rezultatul obtinut de fiecare
		DIE(rc == -1, "cannot wait");
	}

	/* TODO - compute final sum */
	for(i = 0; i < NUM_THREADS; i++) {
		total += *(size_t*)results[i]; //fac totalul
	}

	printf("total = %lu\n", total);

	for (i = 0; i < NUM_THREADS; ++i)
		free(results[i]); //clean up

	munmap(fmap.mem, fmap.size); //demapez zona de memorie
