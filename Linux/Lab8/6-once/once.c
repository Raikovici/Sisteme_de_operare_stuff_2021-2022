...
static int one_time_init(struct once_struct *once_control, void (*f)(void))
{
   ...
   */
	pthread_mutex_lock(&once_control->mutex); //blochez mutexul
	once_control->refcount++; //incrementez counterul
	if(once_control->refcount == 0) //daca am primu thread care intra aici, apelez functia
		(*f)();
	pthread_mutex_unlock(&once_control->mutex); //deblochez mutexu
	return 0;
}
...
static int one_time_deinit(struct once_struct *once_control, void (*f)(void))
{
   ...
   */
	...
	pthread_mutex_lock(&once_control->mutex); //blochez mutexu
	once_control->refcount--; //decrementez counterul
	if(once_control->refcount == 1) //daca e 1, adica ramane un singur mutex care sa intre aici
		(*f)(); //apelez f
	pthread_mutex_unlock(&once_control->mutex); //deblochez mutexu
	return 0;
}
