...
void *do_bad_task(void *arg)
{
	...
	exit(-1); //al patrulea thread va apela functia exit si va termina tot procesu. un thread moare => moare tot procesu
}
...
int main(void)
{
	...
	for (i = 0; i < NUM_TASKS; i++) {
		...
		if(i % 4 == 3) {
			rc = pthread_create(&thread[i], NULL, do_bad_task, &th_id[i]); //fiecare al patrulea thread face un bad task
