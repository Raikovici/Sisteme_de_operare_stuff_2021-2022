...
void acquire_lock(void)
{
	...
	if(__atomic_test_and_set(&lock, __ATOMIC_SEQ_CST) == false) //deci asta verifica daca bool ul a fost 0 si ii modifica valoare, valoarea devine 1 
		return;
	//primitiva atomica, se exec intr un singur ciclu de procesor
	//va pune o valoare pe byte ul non zero si returneaza true daca inainte de a seta valoarea, valoarea veche era tot diferita de 1
	//__ATOMIC_SEQ_CST nu rearanjeaza instructiunile atomice
...
void release_lock(void)
{
	...
	__atomic_clear(&lock, __ATOMIC_SEQ_CST); //reseteaza valoarea lui lock la 0
#else
