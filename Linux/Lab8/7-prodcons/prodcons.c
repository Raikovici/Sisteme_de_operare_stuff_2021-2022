...
void *producer_fn(void *arg)
{
	...
	for (i = 0; i < NR_ITERATIONS; i++) {
		...
		/* TODO - lock mutex */
		pthread_mutex_lock(&mutex); //blochex mutexu

		/* TODO - if common area is full
		 *		then wait until the common area is not full
		 */	
		while(is_buffer_full(&common_area)) { //cat timp bufferul e full
			pthread_cond_wait(&buffer_not_full, &mutex); //astept sa se elibereze din el
		}

		/* TODO - insert item into common area */
		insert_item(&common_area, item_to_insert); //daca e liber in buffer, inserez itemu
		...
		if(common_area.count == 1) {
			pthread_cond_signal(&buffer_not_empty); //daca e un singur element, semnalez ca bufferu nu i gol
		}


		/* TODO - unlock mutex */
		
		pthread_mutex_unlock(&mutex); //deblochez mutexu
...
void *consumer_fn(void *arg)
{
	...
	for (i = 0; i < NR_ITERATIONS; i++) {
		 ...
		 */
		while(is_buffer_empty(&common_area)) { //cat timp e gol
			pthread_cond_wait(&buffer_not_empty, &mutex); //astept sa se semnaleze ca nu i gol
		}

		/* TODO - remove item from common area */
		item_consumed = remove_item(&common_area); //sterg itemu
		 ...
		 */
		if(common_area.count == BUFFER_SIZE - 1) { //daca e aproape plin
			pthread_cond_signal(&buffer_not_full); //semnalez ca bufferu nu i plin, deblochez threadu
		}
